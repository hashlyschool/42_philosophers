...
/*   Updated: 2021/12/11 19:53:01 by hashly           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	*check_time_death(void *arg)
{
	...
	sem_wait(phl->data->sem_check_death);
	if (phl->data->death != 1)
	{
		phl->data->death = 1;
		printf("%lu %d %s", get_time_ms() - phl->data->time_start \
		- START_MS, phl->id, DEID);
	}
	return (NULL);
}
...
void	ft_philo_live(t_data *data, int id)
{
	...
	phl = ft_init_philo_struct(data, id); // нет выделения
	ft_wait_start(&phl); //с ошибками порешали
...
int	main(int argc, char **argv)
{
	...
	ft_init_data(&data, argc, argv); //нет выделения памяти
	ft_init_forks_time(&data); // 2 semapore
	ft_init_philo(&data); //arr_pid //thread create waitpid //child
	ft_exit(&data, 4, "");
}
